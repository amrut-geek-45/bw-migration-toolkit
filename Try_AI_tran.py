...
def _detect_file_key(df):
    ...
    if "DTP"           in cols and "SRC"          in cols and "TGT"        in cols: return "dtp_df"
...
def build_lineage(provider, dfs):
    ...
    def _proc(prov, allow_revisit=False):
        ...
        for r in trfn_rows:
            ...
            # DTP lookup: try (base_s, pu) and (source_up, pu) variants
            source_up = source.upper().strip()
            dtp_list = (
                [d for d in idx_dtp.get((base_s, pu), []) if d.lower() not in ("nan","none","")] or
                [d for d in idx_dtp.get((source_up, pu), []) if d.lower() not in ("nan","none","")] or
                [d for d in idx_dtp.get((base_s, pu.replace("_TR","")), []) if d.lower() not in ("nan","none","")]
            )
...
def build_downstream(obj_name, dfs):
    ...
    for r1 in t1_source_rows:
        ...
        # DTP lookup helper — tries multiple SRC key variants
        def _get_dtp(src_up, tgt_up):
            """Try exact (SRC,TGT), then base (no _TR) variants."""
            src_base = src_up.replace("_TR","").strip()
            tgt_base = tgt_up.replace("_TR","").strip()
            for s, t in [
                (src_up,   tgt_up),
                (src_base, tgt_up),
                (src_up,   tgt_base),
                (src_base, tgt_base),
                # Also try resolved_key as SRC
                (resolved_key or src_up, tgt_up),
                (resolved_key or src_up, tgt_base),
            ]:
                hits = idx_dtp.get((s, t), [])
                if hits:
                    return hits[0]
            return ""

        # DTP for hop 1: DataSource → InfoSource
        dtp1 = _get_dtp(search_up, target1.upper())
        ...
        if target1.upper().endswith("_TR"):
            ...
            if not t2_list:
                ...
                for r2 in t2_list:
                    ...
                    # DTP for hop 2: InfoSource → Provider
                    dtp2    = _get_dtp(target1.upper(), provider.upper())
...
def _build_upstream_rows(query_name, provider, tree, input_type="QUERY"):
    ...
    def _collect_ips(node):
        """Recursively collect all InfoPackage IDs from a node and its lineage."""
        ips = list(node.get("ip_nodes", []))
        for ch in node.get("children", []):
            if "transformation" in ch:
                ips.extend(ch.get("ip_list", []))
                if ch.get("lineage"):
                    ips.extend(_collect_ips(ch["lineage"]))
            else:
                ips.extend(_collect_ips(ch))
        seen = set(); result = []
        for ip in ips:
    ...
    def _walk(children, src_prov="", depth=0):
        """
        Walk children recursively.
        src_prov = the InfoCube/ADSO that is the direct child of the root provider
                   (e.g. when root = MultiProvider, src_prov = InfoCube/ADSO inside it)
        """
        for child in children:
            if "transformation" not in child:
                # Non-transformation node (MultiProvider part, ADSO etc.)
                # Pass this node's name as src_prov for its children
                child_src = child.get("provider", "")
                _walk(child.get("children", []), src_prov=child_src, depth=depth+1)
                continue
            ...
            # Walk deeper to find is_trfn (InfoSource→DS transformation) and DataSource
            is_trfn = ""; datasource = ""
            if lineage:
            ...
            # Collect InfoPackages — try ip_list first, then full recursive search
            if not ip_list and lineage:
                ip_list = _collect_ips(lineage)
            ...
            groups.append({
                ...
                "src_prov":  src_prov,  # InfoCube/ADSO inside MultiProvider
            })

    # Determine initial src_prov:
    # If root provider is a MultiProvider, src_prov = its parts (InfoCubes/ADSOs)
    # If root is directly InfoCube/ADSO, src_prov = "" (same as root provider)
    root_type = tree.get("type","") if tree else ""
    if root_type == "MULTIPROVIDER":
        # Walk with src_prov set per MultiProvider part
        _walk(tree.get("children", []) if tree else [], src_prov="")
    else:
        # Root IS the provider — no separate src_prov needed
        _walk(tree.get("children", []) if tree else [], src_prov="")
    ...
    for g in groups:
        ...
        for k, ip in enumerate(g["ips"]):
            flat_rows.append({"values": [
                query_name      if (len(flat_rows) == 0) else "",  # 0 BEx Name
                provider        if (len(flat_rows) == 0) else "",  # 1 Root Provider
                g["src_prov"]   if k == 0 else "",                 # 2 Source for Root Provider
                g["trfn"]       if k == 0 else "",                 # 3 Transformation (Prov→InfoSrc)
                g["infosource"] if k == 0 else "",                 # 4 InfoSource
                g["dtp_list"][0] if (k == 0 and g["dtp_list"]) else "",  # 5 InfoSource DTP
                g["is_trfn"]    if k == 0 else "",                 # 6 Transformation (InfoSrc→DS)
                g["ds"]         if k == 0 else "",                 # 7 DataSource
                ip,                                                # 8 InfoPackage
            ], "fill": fill})
        ...
        if span > 1:
            ...
            # Merge cols 3-8 (Transformation through DataSource) per group
            merge_spec.append((er_s, er_e, [3,4,5,6,7,8], fill))

    if len(flat_rows) > 1:
        # Merge BEx Name and Root Provider across all rows
...
def _write_upstream_sheet(ws, query_name, provider, tree, input_type="QUERY"):
    ...
    # ── QUERY path: build rows first, then detect which columns have data
    flat_rows, merge_specs = _build_upstream_rows(query_name, provider, tree, input_type)
    ...
    # All possible columns with their position (0-indexed) and widths
    ALL_COLS = [
        ("BEx Name",                       0, 28),
        ("Root Provider",                  1, 20),
        ("Source for Root Provider",        2, 22),
        ("Transformation (Prov→InfoSrc)",  3, 40),
        ("InfoSource",                     4, 22),
        ("InfoSource DTP",                 5, 38),
        ("Transformation (InfoSrc→DS)",    6, 40),
        ("DataSource",                     7, 22),
        ("InfoPackage",                    8, 38),
    ]

    # Detect which column indices have at least one non-empty value
    active_indices = set()
    for row in flat_rows:
        for ci, val in enumerate(row["values"]):
            if val and str(val).strip():
                active_indices.add(ci)

    # Always include BEx Name (0) and Root Provider (1) if QUERY type
    active_indices.add(0)
    active_indices.add(1)

    # Build dynamic column list — only columns with data
    dyn_cols   = [(name, orig_ci, w) for name, orig_ci, w in ALL_COLS if orig_ci in active_indices]
    ...
    # Write rows — remap values to only active columns
    for ri, row in enumerate(flat_rows):
    ...
    # Remap merge specs — translate old column indices to new column indices
    for (er_s, er_e, orig_cols, fill) in merge_specs:
...
def _write_downstream_sheet(ws, downstream):
    ...
    for chain in downstream:
        ...
        for ri in range(total):
            flat_dn.append({"values": [
                ...
                chain["dtp"]               if ri == 0 else "",         # 5 DTP
                chain["provider"]          if ri == 0 else "",         # 6 InfoProvider
                qs[ri]                     if ri < len(qs)  else "",   # 7 BEx Query
            ], "fill": fill})
        c_end = len(flat_dn) - 1
        if c_end > c_start:
    ...
    for row in flat_dn:
        for ci, val in enumerate(row["values"]):
            if val and str(val).strip():
                active_idx.add(ci)
    # Always include DataSource (1) and BEx Query (7)
    active_idx.update([1, 7])
...
def tree_to_txt_download(query_name, provider, tree, stats, downstream=None, input_type="QUERY"):
    ...
    if input_type == "QUERY":
        ...
        def _write_node(node, pad="", is_last=True):
            ...
            dtps = node.get("dtps", [])
            if dtps:
                lines.append(f"{child_pad}{B}DTPs : {len(dtps)}")
                for d in dtps:
                    lines.append(f"{child_pad}{P}  {L}{d}")
            ...
            for ci, child in enumerate(children):
                ...
                if "transformation" in child:
                    ...
                    lines.append(f"{cp}  {B}DTPs : {len(dl)}")
                    for d in dl:
                        lines.append(f"{cp}  {P}  {L}{d}")
        ...
        for ci, chain in enumerate(downstream):
            ...
            if chain.get("dtp"):
                lines.append(f"  {B}DTP                 : {chain['dtp']}")
