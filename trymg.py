...
def load_metadata():
    ...
    # Clean client suffix from transformation names
...
def build_lineage(provider, dfs):
    ...
    def _proc(prov):
        ...
        # Transformations targeting this node
        ...
        # MultiProvider → part providers
        if node["type"] == "MULTIPROVIDER" and mp_df is not None:
            for cube in mp_df[
                mp_df["MULTIPROVIDER"].str.upper().eq(pu)
            ]["PARTPROVIDER"].dropna().unique():
                ch = _proc(str(cube).strip())
                if ch:
                    node["children"].append(ch)

        # Transformations → upstream chain
        if trfn_df is not None and "TARGETNAME" in trfn_df.columns:
            for _, row in trfn_df[trfn_df["TARGETNAME"].str.upper().eq(pu)].iterrows():
                ...
                if dtp_df is not None and "SRC" in dtp_df.columns and "TGT" in dtp_df.columns:
                    dm2 = dtp_df[
                        dtp_df["SRC"].str.upper().eq(base_s.upper()) &
                        dtp_df["TGT"].str.upper().eq(pu)
                    ]
                    if not dm2.empty:
...
# =====================================================
# FLAT ROW BUILDER
# Converts the tree into a flat list of visual rows
# with correct depth, type, name — used by render_lineage
# =====================================================
...
def flatten_tree(node, rows=None, depth=0):
    ...
    otype = node.get("type", "DATASOURCE")
    rows.append({"type": otype, "name": node["provider"], "depth": depth})

    # Transformation summary list on the provider
    for tr in node.get("transformations", []):
        rows.append({"type": "TRANSFORMATION", "name": tr, "depth": depth + 1})

    # DTPs on the provider
    for dtp in node.get("dtps", []):
        rows.append({"type": "DTP", "name": dtp, "depth": depth + 1})

    # InfoPackages on the provider
...
# =====================================================
# VISUAL RENDERER  — colored cards with indentation
# =====================================================
...
def render_lineage_visual(rows):
    ...
    for otype in seen:
        cls, icon, label = _TYPE_META.get(otype, ("ln-default", "📦", otype))
        legend.append(
            f"<span class='ln-row {cls} ln-leg-item'>"
            f"{icon} {label}</span>"
        )
    st.markdown(
        "<div class='ln-legend'>" + "".join(legend) + "</div>",
        unsafe_allow_html=True
    )


def tree_to_text(tree):
    """Plain-text tree for download."""
    lines  = []
    indent = "    "
    B, L, P = "├── ", "└── ", "│   "
    ...
    def _walk(node, level=0):
        ...
        for child in node.get("children", []):
            if "transformation" in child:
                ...
                lines.append(f"{pad}{P}  {B}DTPs : {len(dtp_list)}")
                for d in dtp_list:
                    lines.append(f"{pad}{P}  {P}  {B}{d}")
                ip_list = child.get("ip_list", [])
                lines.append(f"{pad}{P}  {L}InfoPackages : {len(ip_list)}")
                for i, ip in enumerate(ip_list):
                    lines.append(f"{pad}{P}      {'└── ' if i==len(ip_list)-1 else '├── '}{ip}")
                if child.get("lineage"):
...
def do_risk_analysis(query_input, provider, tree, stats):
    ...
    prompt = (
        ...
        "\"risks\":[{\"level\":\"HIGH\",\"object\":\"obj\","
        "\"issue\":\"problem\",\"recommendation\":\"fix\"}],"
        "\"migration_effort_days\":10,\"key_actions\":[\"a1\",\"a2\"]}\n"
        "Levels: CRITICAL, HIGH, MEDIUM, LOW only. migration_effort_days: integer.\n\n"
        f"Lineage:\n{ctx}"
    )
    raw = call_claude(
...
def do_chat(question, ctx, history):
    system = ("You are an SAP BW/4HANA lineage expert. Answer questions about "
              "the lineage concisely and accurately.")
    messages = []
    for h in history:
...
def collect_all_objects(dfs):
    """object_name(upper) -> set of query IDs that use it"""
    ...
    for _, row in query_df.iterrows():
        ...
        def _collect(prov):
            ...
            if dfs.get("mp_df") is not None and "PARTPROVIDER" in dfs["mp_df"].columns:
                for part in dfs["mp_df"][
                    dfs["mp_df"]["MULTIPROVIDER"].str.upper().eq(pu)
                ]["PARTPROVIDER"].dropna():
...
def do_impact_analysis(obj_name, dfs):
    ...
    partial  = {obj: sorted(qs) for obj,qs in obj_map.items()
...
def do_query_comparison(q1, q2, dfs):
    def _get(q):
        ...
        m = qdf[
            qdf["QUERYID"].str.contains(q,case=False,na=False) |
            qdf["QUERYNAME"].str.contains(q,case=False,na=False)
...
# ─────────────────────────────────────────
# LEFT — Upload & Transformation Analysis
# ─────────────────────────────────────────
with left_col:
    st.markdown("#### 📂 Transformation Analyzer")

    # ── Input mode: single file OR csv list of files
    lt1, lt2 = st.tabs(["📁 Single File", "📋 Batch via CSV / Excel"])

    uploaded_file  = None
    batch_files    = []          # list of UploadedFile objects
    ...
    with lt2:
        st.caption(
            "Upload **multiple** transformation Excel files at once, "
            "or a **CSV/Excel list** with a column named `FILENAME` pointing to file names. "
            "You can also just select multiple .xlsx files directly."
        )
        batch_files = st.file_uploader(
            "Transformation files (multi-select)",
            type=["xlsx","xls"],
    ...
    # ── Helper: analyze one file, return result dict
    def _analyze_one(uf):
        try:
            uf.seek(0)
    ...
    if result and not st.session_state.trfn_results:
        ...
        ztable_cnt = len(result.get("ztables",[])) if result.get("ztables") else 0
        m1.metric("📄 Code Lines",  result.get("code_lines","—"))
        m2.metric("🗂️ Z-Tables",    ztable_cnt)
    ...
    if st.session_state.trfn_results:
        ...
        for r in st.session_state.trfn_results:
            if r["error"]:
                summary_rows.append({"File": r["file"], "Complexity": "ERROR",
                                     "Z-Tables": "", "Error": r["error"]})
            else:
                res = r["result"]
                summary_rows.append({
                    "File":       r["file"],
        ...
        for r in st.session_state.trfn_results:
            with st.expander(f"{'❌' if r['error'] else '✅'} {r['file']}"):
                if r["error"]:
                    ...
                    res = r["result"]
                    cx  = str(res.get("complexity","?")).upper()
                    st.markdown(f"**Complexity:** `{cx}`")
...
with right_col:
    ...
    st.caption(
        "Full chain — Query → InfoCube/ADSO → Transformation → "
        "InfoSource → DTP → DataSource → InfoPackages"
    )

    # Metadata status
    loaded = sum(1 for k,v in dfs.items() if not k.startswith("_") and v is not None)
    if loaded == 7:
        st.success("✅ All 7 metadata files loaded (OBJVERS = A only)")
    elif loaded > 0:
        st.warning(f"⚠️ {loaded}/7 metadata files loaded")
    else:
        st.error("❌ No metadata files found in Meta_Data_File folder")
    ...
    with inp_tab2:
        st.caption(
            "Upload a CSV or Excel file with a **QUERYID** or **QUERYNAME** column "
            "(or any single-column file — first column is used)."
        )
        qfile = st.file_uploader(
            "Query list file", type=["csv","xls","xlsx"],
        ...
        if qfile:
            try:
                ...
                if qf_df is not None and len(qf_df) > 0:
                    ...
                    queries_to_run = [q.strip() for q in raw_qs
                                      if q.strip() and q.strip().lower() != "nan"]
    ...
    if trace and queries_to_run:
        ...
        if query_df is None:
            ...
            for idx, q in enumerate(queries_to_run):
                prog.progress((idx) / total, text=f"Tracing: {q}")
                mask = pd.Series(False, index=query_df.index)
                if "QUERYID"   in query_df.columns:
                    mask = mask | query_df["QUERYID"].str.contains(q, case=False, na=False)
                if "QUERYNAME" in query_df.columns:
                    mask = mask | query_df["QUERYNAME"].str.contains(q, case=False, na=False)
            ...
            if results:
                # Single query → use existing single-result state (AI tabs work)
                if len(results) == 1:
                    r = results[0]
                    ...
                    st.session_state.lin_results  = None   # single mode
                else:
                    # Multiple → store all results; single-query AI tabs hidden
                    st.session_state.lin_results  = results
                    st.session_state.lin_tree     = None   # suppress single view
                    st.session_state.lin_error    = None
                # Reset AI
    ...
    # ── BATCH render — multiple queries from CSV upload
        ...
        for r in results:
            ...
            rows.append({"Query": r["query"], "Provider": r["provider"],
                         "Complexity": cx, "Score": score,
                         "Transforms": t, "DTPs": d, "InfoPkgs": i})
        st.dataframe(pd.DataFrame(rows), use_container_width=True)

        # Bulk ZIP download
        import zipfile as _zf2
        zip_buf2 = io.BytesIO()
        with _zf2.ZipFile(zip_buf2, "w", _zf2.ZIP_DEFLATED) as zf2:
            for r in results:
        ...
        for r in results:
            ...
            with st.expander(f"🔍 {r['query']}  —  {cx}  (score {score})", expanded=False):
                ...
                rows_vis = flatten_tree(r["tree"])
                render_lineage_visual(rows_vis)
                txt2 = tree_to_text(r["tree"])
                safe = "".join(c if c.isalnum() or c in "-_" else "_" for c in r["query"])
                st.download_button("⬇️ Download this lineage",
                                   data=txt2.encode(),
        ...
        with tab_lin:
            ...
            st.download_button("⬇️ Download Lineage (.txt)", data=txt.encode(),
        ...
        with tab_risk:
            if not ai_risk_on:
                ...
                if rd is None:
                    ...
                    with rc1:
                        st.markdown(
                            f'<div class="risk-banner-{overall}">'                            f'<b style="font-size:15px">{icon} Overall Risk: {overall}</b>'                            f'<br><span style="font-size:13px;opacity:.85">{rd.get("summary","")}</span></div>',
                            unsafe_allow_html=True
                        )
                    with rc2:
                    ...
                    for r in rd.get("risks",[]):
                        ...
                        st.markdown(
                            f'<div class="risk-{lvl}">'                            f'<b>{ICONS.get(lvl,"⚪")} [{lvl}] {r.get("object","")}</b>'                            f'<br>🔍 {r.get("issue","")}'                            f'<br>✅ <i>{r.get("recommendation","")}</i></div>',
                            unsafe_allow_html=True
                        )
    ...
    # ── AI TOOLS (Impact, Dead Objects, Comparison) — always visible
    st.markdown("---")
    st.markdown("#### 🛠️ AI Tools")

    tool_tab1, tool_tab2, tool_tab3 = st.tabs(
        ["🔎 Impact Analysis", "🧹 Dead Objects", "⚖️ Query Comparison"]
    )
    ...
    with tool_tab1:
        if not ai_ia_on:
            ...
            if res:
                ...
                lines = [f"IMPACT: {res['object']}", "="*50,
                         f"Direct: {len(res['impacted'])}"] + [f"  {q}" for q in res["impacted"]]
                lines += ["", "AI:", res["ai"]]
                ia_report = "\n".join(lines)
    ...
    with tool_tab2:
        if not ai_dead_on:
            ...
            if res:
                ...
                dc3.metric("🗑️ Cleanup %",
                           f"{round(len(res['dead'])/(len(res['dead'])+res['active_count'])*100) if (len(res['dead'])+res['active_count'])>0 else 0}%")
            ...
            if res:
                if "error" in res:
                    ...
                    lines = [f"COMPARISON: {res['q1']} vs {res['q2']}", "="*60,
                             ...
                             f"Shared ({len(res['shared'])}):"]+[f"  {o}" for o in res["shared"]]
