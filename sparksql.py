...
.lang-sql{background:#0d1f2d;border-left:4px solid #38bdf8;padding:12px 14px;
    border-radius:6px;font-size:13px;color:#c9d1d9;margin:6px 0}
.lang-spark{background:#1a0f2e;border-left:4px solid #a78bfa;padding:12px 14px;
    border-radius:6px;font-size:13px;color:#c9d1d9;margin:6px 0}
.trans-header{font-size:13px;font-weight:600;color:var(--color-text-primary);
...
# =====================================================
# TRANSFORMATION CODE TRANSLATOR
# =====================================================

def do_translate_to_sql(abap_code, field_summary):
    """Translate SAP BW transformation ABAP/rule code to SQL."""
    summary_text = ""
    if field_summary is not None:
        try:
            summary_text = field_summary.to_string(index=False, max_rows=50)
        except:
            pass
    prompt = (
        "You are an expert in SAP BW transformations and SQL. "
        "Convert the following SAP BW transformation code/rules to standard SQL. "
        "Rules: use standard ANSI SQL (SELECT, FROM, WHERE, CASE WHEN, JOIN). "
        "Map SAP field names to SQL column names as-is. "
        "Replace ABAP-specific functions with SQL equivalents. "
        "Add inline comments explaining SAP-specific logic. "
        "Assume source table = SOURCE_TABLE, target table = TARGET_TABLE. "
        "If field mapping summary is provided, use it to build the SELECT columns. "
        "Return ONLY the SQL code with inline comments. No explanation before or after.\n\n"
        f"Field Mapping Summary:\n{summary_text}\n\n"
        f"Transformation Code:\n{abap_code}"
    )
    return call_claude(
        "You are an SAP BW to SQL migration expert. Return only clean SQL code.",
        prompt,
        max_tokens=2000
    )


def do_translate_to_pyspark(abap_code, field_summary):
    """Translate SAP BW transformation ABAP/rule code to PySpark."""
    summary_text = ""
    if field_summary is not None:
        try:
            summary_text = field_summary.to_string(index=False, max_rows=50)
            ...
            pass
    prompt = (
        "You are an expert in SAP BW transformations and Apache Spark. "
        "Convert the following SAP BW transformation code/rules to PySpark (DataFrame API). "
        "Rules: use PySpark DataFrame API (not RDD). "
        "Import all required pyspark.sql.functions at the top. "
        "Map SAP field names to PySpark column names as-is. "
        "Replace ABAP-specific functions with PySpark equivalents. "
        "Add inline comments explaining SAP-specific logic. "
        "Assume source DataFrame = source_df, result stored in target_df. "
        "Use withColumn(), select(), when(), col(), lit(), coalesce() etc. as needed. "
        "If field mapping summary is provided, use it to build the transformation. "
        "Return ONLY the PySpark code with inline comments. No explanation before or after.\n\n"
        f"Field Mapping Summary:\n{summary_text}\n\n"
        f"Transformation Code:\n{abap_code}"
    )
    return call_claude(
        "You are an SAP BW to PySpark migration expert. Return only clean PySpark code.",
        prompt,
        max_tokens=2000
    )


def do_explain_translation_diff(abap_code, sql_code, spark_code):
    """AI explains key differences and migration notes."""
    prompt = (
        f"Original SAP BW ABAP/Rule code (first 300 chars):\n{abap_code[:300]}\n\n"
        f"SQL translation (first 300 chars):\n{sql_code[:300]}\n\n"
        f"PySpark translation (first 300 chars):\n{spark_code[:300]}\n\n"
        "Write 4-6 bullet points covering: "
        "key logic differences between ABAP and the translations; "
        "any SAP-specific functions that needed special handling; "
        "performance considerations for SQL vs PySpark; "
        "things the developer should manually verify after translation. "
        "Keep each bullet to 1-2 sentences. Start each with a bullet symbol."
    )
    return call_claude(
        "You are an SAP BW migration expert specialising in code translation.",
        prompt,
        max_tokens=500
    )
...
for _k, _v in [
    ...
    ("trans_sql",None),("trans_spark",None),
    ("trans_notes",None),("trans_code",""),("trans_summary",None),
]:
...
with left_col:
    ...
    lt1, lt2, lt3 = st.tabs(["📁 Single File", "📋 Batch via CSV / Excel", "🔄 Code Translator"])
    uploaded_file = None
    batch_files   = []
    ...
    with lt3:
        st.caption(
            "Paste your SAP BW transformation ABAP/rule code — or upload a transformation "
            "Excel and the code will be extracted automatically. "
            "AI translates it to **SQL** and **PySpark**."
        )

        # ── Source: paste OR auto-extract from uploaded file
        src_tab1, src_tab2 = st.tabs(["✍️ Paste Code", "📎 From Uploaded File"])

        with src_tab1:
            pasted_code = st.text_area(
                "Paste ABAP / transformation rule code",
                height=180,
                placeholder="PROCEDURE abap_source_fields...\nENDPROCEDURE.",
                key="trans_paste",
                label_visibility="collapsed"
            )
            if pasted_code.strip():
                st.session_state.trans_code    = pasted_code.strip()
                st.session_state.trans_summary = None

        with src_tab2:
            st.caption("Automatically uses code from the Single File tab if a file was already uploaded.")
            if st.session_state.result and st.session_state.data:
                code_preview = str(st.session_state.data.get("code", ""))[:500]
                st.text_area("Extracted code preview",
                             value=code_preview + ("..." if len(code_preview)==500 else ""),
                             height=100, disabled=True, label_visibility="collapsed")
                if st.button("Use this code for translation", key="use_uploaded_code"):
                    st.session_state.trans_code    = str(st.session_state.data.get("code",""))
                    st.session_state.trans_summary = st.session_state.data.get("summary")
                    st.session_state.trans_sql     = None
                    st.session_state.trans_spark   = None
                    st.session_state.trans_notes   = None
                    st.success("✅ Code loaded from uploaded transformation file")
            else:
                st.info("Upload a transformation file in the 📁 Single File tab first.")

        # ── Code loaded indicator
        if st.session_state.trans_code:
            lines_count = len(st.session_state.trans_code.splitlines())
            st.markdown(
                f"<div style='background:#0f2a1e;border-left:3px solid #3fb950;"
                f"padding:6px 12px;border-radius:4px;font-size:12px;color:#7ee787;margin:6px 0'>"
                f"✅ Code ready: <b>{lines_count} lines</b> loaded for translation</div>",
                unsafe_allow_html=True
            )
        ...
        with tc1:
            run_sql = st.button("🔷 Translate to SQL", type="primary",
                                use_container_width=True, key="run_sql",
                                disabled=not bool(st.session_state.trans_code))
        with tc2:
            run_spark = st.button("🟣 Translate to PySpark", type="primary",
                                  use_container_width=True, key="run_spark",
                                  disabled=not bool(st.session_state.trans_code))
        with tc3:
            run_both = st.button("⚡ Translate Both", use_container_width=True,
                                 key="run_both",
                                 disabled=not bool(st.session_state.trans_code))
        ...
        if run_sql or run_both:
            with st.spinner("🔷 Translating to SQL…"):
                st.session_state.trans_sql = do_translate_to_sql(
                    st.session_state.trans_code,
                    st.session_state.trans_summary
                )
            st.session_state.trans_notes = None  # reset notes

        if run_spark or run_both:
            with st.spinner("🟣 Translating to PySpark…"):
                st.session_state.trans_spark = do_translate_to_pyspark(
                    st.session_state.trans_code,
                    st.session_state.trans_summary
                )
            st.session_state.trans_notes = None
        ...
        # ── Show results
        if st.session_state.trans_sql or st.session_state.trans_spark:
            st.markdown("---")
            res_tab1, res_tab2, res_tab3 = st.tabs(
                ["🔷 SQL", "🟣 PySpark", "📝 Migration Notes"]
            )

            with res_tab1:
                if st.session_state.trans_sql:
                    sql_val = st.session_state.trans_sql
                    if sql_val.startswith("[ERROR"):
                        st.error(sql_val)
                    else:
                        ...
                        with col_dl:
                            st.download_button(
                                "⬇️ Download .sql", data=sql_val.encode(),
                                file_name="transformation.sql", mime="text/plain",
                                key="dl_sql"
                            )
                        with col_regen:
                            if st.button("🔄 Regenerate SQL", key="regen_sql"):
                                st.session_state.trans_sql = None; st.rerun()
                else:
                    st.info("Click '🔷 Translate to SQL' or '⚡ Translate Both' above.")

            with res_tab2:
                if st.session_state.trans_spark:
                    spark_val = st.session_state.trans_spark
                    if spark_val.startswith("[ERROR"):
                        st.error(spark_val)
                    else:
                        st.code(spark_val, language="python")
                        col_dl2, col_regen2 = st.columns([1, 1])
                        with col_dl2:
                            st.download_button(
                                "⬇️ Download .py", data=spark_val.encode(),
                                file_name="transformation_spark.py", mime="text/plain",
                                key="dl_spark"
                            )
                        with col_regen2:
                            if st.button("🔄 Regenerate PySpark", key="regen_spark"):
                                st.session_state.trans_spark = None; st.rerun()
                else:
                    st.info("Click '🟣 Translate to PySpark' or '⚡ Translate Both' above.")

            with res_tab3:
                if not st.session_state.trans_sql and not st.session_state.trans_spark:
                    st.info("Run at least one translation first.")
                else:
                    notes = st.session_state.get("trans_notes")
                    if notes is None:
                        if st.button("📝 Generate Migration Notes", key="gen_notes", type="primary"):
                            with st.spinner("📝 Generating notes…"):
                                st.session_state.trans_notes = do_explain_translation_diff(
                                    st.session_state.trans_code,
                                    st.session_state.trans_sql   or "",
                                    st.session_state.trans_spark or ""
                                )
                            st.rerun()
                        ...
                        st.markdown(
                            f'<div class="ai-sum-box">📝 <b>Migration Notes</b><br><br>'
                            f'{notes.replace(chr(10), "<br>")}</div>',
                            unsafe_allow_html=True
                        )
                        if st.button("🔄 Regenerate Notes", key="regen_notes"):
                            st.session_state.trans_notes = None; st.rerun()

        # ── Clear all translations
        if (st.session_state.trans_sql or st.session_state.trans_spark
                or st.session_state.trans_code):
            st.markdown("")
            if st.button("🗑️ Clear all translations", key="clear_trans"):
                st.session_state.trans_sql     = None
                st.session_state.trans_spark   = None
                st.session_state.trans_notes   = None
                st.session_state.trans_code    = ""
